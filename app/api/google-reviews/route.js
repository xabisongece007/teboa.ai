...
async function loadPlacesReviews(apiKey, placeId) {
  ...
  if (!response.ok || payload.status !== "OK") {
    throw new Error(
      payload.error_message || payload.status || "Google Places request failed."
    );
  }
...
export async function GET() {
  ...
  if (hasBusinessProfileConfig) {
    try {
      return json(200, await loadBusinessProfileReviews(businessProfileConfig));
    } catch (error) {
      return json(502, {
        configured: true,
        source: "business-profile",
  ...
  if (apiKey && placeId) {
    try {
      return json(200, await loadPlacesReviews(apiKey, placeId));
    } catch (error) {
      return json(502, {
        configured: true,
        source: "places",
